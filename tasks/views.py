...
def index(request):
    ...
    html = f"<h1>Страница приложения tasks</h1><a href='{quality_control_url}'>Перейти к системе контроля качества</a><br><a href='{projects_list_url}'>Список проектов</a>"
...
class IndexView(View):
    def get(self, request, *args, **kwargs):
        projects_list_url = reverse('tasks:projects_list')
        quality_control_url = reverse('quality_control:index')
        html = f"<h1>Страница приложения tasks</h1><a href='{projects_list_url}'>Список проектов</a><br><a href='{quality_control_url}'>Перейти к системе контроля качества</a>"
        return HttpResponse(html)
